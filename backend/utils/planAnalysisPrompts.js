...
- 쇼핑몰: 최종 고객(회원/비회원)이 직접 사용하는 화면으로, 이 프로젝트 자체가
  상품을 진열하고 장바구니에 담아 결제(PG 연동)까지 직접 처리하는 서비스.
  화면 안에 "BO", "Admin", "관리자 등록" 같은 단어가 일부 언급되더라도,
  그 화면 자체가 "고객이 보는 화면(FO)"이면 쇼핑몰로 분류해야 함.
- 포인트/멤버십형 서비스: 최종 고객이 직접 사용하는 화면이지만, 실제 상품 판매나
  결제(PG 연동)는 이 프로젝트가 직접 처리하지 않고, 포인트 적립/차감/소멸,
  쿠폰, 응모/당첨, 혜택 지급이 핵심 기능인 서비스. "결제금액"이라는 단어가
  등장하더라도, 그게 다른 서비스(입점몰 등)에서 이미 발생한 결제/구매의
  결과값을 포인트 적립 조건으로 참조만 하는 것이라면(이 프로젝트가 장바구니-결제
  화면 자체를 갖고 있지 않다면) 쇼핑몰이 아니라 이 유형으로 분류해야 함.
...
쇼핑몰과 포인트/멤버십형 서비스를 가를 때는 이 기준을 최우선으로 써줘:
"이 화면에서 고객이 상품을 직접 장바구니에 담고 결제(PG)까지 진행하는가?"
→ 그렇다면 쇼핑몰. 결제/구매는 다른 곳(입점몰 등)에서 끝나고, 이 프로젝트는
포인트 적립·쿠폰·응모·당첨 같은 혜택만 다룬다면 → 포인트/멤버십형 서비스.
단순히 포인트/쿠폰/응모 기능이 있다는 이유만으로 쇼핑몰로 분류하지 마.

예시로 판단 기준을 명확히 할게:
- "고객이 직접 상품을 장바구니에 담고 결제(PG)까지 진행하는 화면"은 쇼핑몰임.
- "관리자가 등록한 쿠폰/이벤트/포인트 혜택에, 고객이 응모하거나 다운로드하거나
  참여해서 당첨/보상을 받는 화면"이면서, 그 상품/결제 자체는 이 프로젝트 밖(입점몰 등)
  에서 이뤄지는 경우는 관리자 언급이 있어도 "포인트/멤버십형 서비스"로 분류해야 함.
  (예: 이벤트 응모, 쿠폰 다운로드, 포인트 적립/차감, 당첨자 발표를 고객이 직접 보는 화면인데
  실제 상품 결제는 이 프로젝트가 처리하지 않는 경우)
  이런 화면은 관리자가 "백엔드에서 무언가를 등록/관리"한다는 설명이 있어도,
  실제 화면의 사용자(응모하기 버튼을 누르는 사람)는 최종 고객이므로 내부 관리 시스템은 아님.
- "관리자가 로그인해서 직접 쿠폰을 등록하거나, 회원 목록을 조회/수정하는 화면 자체"를
  설계한 문서일 때만 내부 관리 시스템으로 분류해야 함.

체크리스트: 이 화면에서 버튼을 누르는 주체가 고객이면서 상품 결제(PG)까지 이 프로젝트가
직접 처리하면 → 쇼핑몰. 고객이 버튼을 누르지만 결제는 이 프로젝트 밖에서 끝나 있고
포인트/쿠폰/응모/당첨만 다루면 → 포인트/멤버십형 서비스.
...
내부 관리 시스템 → "관리 시스템", 포인트/멤버십형 서비스 → "멤버십 서비스", 기타 → "서비스".
...
function buildRequirementChecklistPrompt() {
  return `위에서 제공된 기획서 내용과 참고 체크리스트를 바탕으로 통합 리스트를 만들어줘.

각 체크리스트 항목마다 먼저, 이 항목이 다루는 기능을 이 프로젝트가 실제로 "직접 수행/소유"하는지
판단해줘.
- 예: "결제 PG사 응답/콜백 처리"는, 이 프로젝트가 결제 게이트웨이(PG)와 직접 연동해서
  승인/실패/콜백을 처리하는 경우에만 해당하는 항목이야.
- 기획서에 "결제금액" 같은 단어가 등장하더라도, 그게 외부(다른 서비스/입점몰)에서 이미
  처리된 결제의 결과값을 포인트 적립 등 다른 기능의 조건값으로 참조만 하는 거라면,
  이 프로젝트가 그 기능을 직접 소유하는 게 아니야.
- 이렇게 이 프로젝트의 기능 범위 밖인 체크리스트 항목은 [충족]도 [누락 의심]도 아니니,
  아예 출력하지 말고 건너뛰어줘.

이 프로젝트가 실제로 소유하는 기능에 대해서만, 기획서에 명시적으로 있으면 [충족],
체크리스트에는 있지만 기획서에 언급이 없거나 애매하면 [누락 의심]으로 표시해줘.
...
각 TC를 만들 때마다, 아래 "이 프로젝트만의 규칙" 목록을 하나씩 순서대로 확인해줘: 지금 만드는 TC의
제목/사전조건/절차/기대결과 중 하나라도 그 규칙의 내용과 실질적으로 겹치면(같은 조건, 같은 예외
상황을 다루면), 절대 빠뜨리지 말고 그 규칙의 요약을 [BASED_ON_RULE]에 적어야 해. 이건 선택이 아니라
필수 확인 절차야 - 겹치는 규칙이 있는데도 [BASED_ON_RULE]을 비워두면 안 돼. 정말 어떤 규칙과도
무관한 TC일 때만 빈 값으로 둬.
